...
const SinglePlant = () => {
  ...
  const { loading, error, data } = useQuery(QUERY_SINGLE_PLANT, {variables: {plantId: plantId}});
  console.log(data);
  if (!data) {
      return <p>Loading...</p>;
  };
  ...
  return (
    <div id="singlePlantPage">
      <div id="plantWrapper">
        ...
        <img className="pImage" src={plantData.plantImg} alt={plantData.plantName} />
        <ul className="pDetails">
            ...
            <li>Weeks before(-)/after last Spring frost to sow outdoors: {plantData.outdoorStartCalc}</li>
        </ul>
        <div id="btnWrapper">
          {/* require login to click these buttons */}
          <button class="plantBtn" id="editBtn" type="button">Edit Plant</button>
          ...
          <button class="plantBtn" id="deleteBtn" type="submit">Delete</button>
        </div>
      </div>
    </div>
  )
